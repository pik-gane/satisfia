...
from environments.very_simple_gridworlds import all_worlds, make_simple_gridworld
...
gridworld_dropdown = sg.DropDown(
    gridworlds, default_value=default_gridworld, key="gridworld_dropdown"
)

override_aleph_checkbox = sg.Checkbox(
    "Override aleph0", default=False, key="override_aleph_checkbox", enable_events=True
)
...
for pd in parameter_data:
    parameter_sliders[pd[0]] = sg.Slider(
        range=(pd[1], pd[2]),
        default_value=pd[3],
        resolution=pd[4],
        orientation="h",
...
autorestart_checkbox = sg.Checkbox(
    "Auto restart", default=True, key="autorestart_checkbox"
)

speed_slider = sg.Slider(
    range=(1, 20), default_value=10, orientation="h", key="speed_slider"
)
    ...
    [
        sg.Column(
            [
                [
                    sg.Text(
                        parameter_data[2 * r][0],
                        size=(state, None),
                        justification="right",
                    ),
                    parameter_sliders[parameter_data[2 * r][0]],
                    sg.Text(
                        parameter_data[2 * r + 1][0],
                        size=(state, None),
                        justification="right",
                    ),
                    parameter_sliders[parameter_data[2 * r + 1][0]],
                ]
...
def step():
    global \
        ...
        initialMu0, \
        initialMu20, \
        visited_state_alephs, \
        visited_action_alephs
    print()
    ...
    if values["lossCoeff4WassersteinTerminalState"] != 0:
        ...
        for a in agent.world.possible_actions(state):
            ...
            print(
                "      actual  ETerminalState_state (s) :",
                list(agent.ETerminalState_action(state, a, al4a, "actual")),
            )
            print(
                "      actual  ETerminalState2_state(s) :",
                list(agent.ETerminalState2_action(state, a, al4a, "actual")),
            )
            print(
                "      --> Wasserstein distance",
                agent.wassersteinTerminalState_action(state, a, al4a),
            )
            print(
                "      expected Total (state aleph):",
                agent.Q(state, a, al4a),
                f"({aleph})",
            )
        ...
        if values["autorestart_checkbox"]:
            ...
            for state, aleph in visited_state_alephs:
                (
                    ...
                    mv_locs,
                    mv_states,
                ) = env._extract_state_attributes(state)
                if loc not in Vs:
                    Vs[loc] = []
                Vs[loc].append(
                    f"{aleph[0]},{aleph[1]}:{agent.V(state, aleph)}"
                )  # expected Total
            Qs = {}
            for state, action, aleph in visited_action_alephs:
                ...
                Qs[key].append(
                    f"{aleph[0]},{aleph[1]}:{agent.relativeQ2(state, action, aleph, agent.Q(state, action, aleph))}"
                )  # variance of Total

            env.render(additional_data={"cell": Vs, "action": Qs})
    else:
        print(
            "t:",
            t,
...
def reset_env(start=False):
    ...
    if gridworld != old_gridworld:
        env, aleph0 = make_simple_gridworld(
            gw=gridworld, render_mode="human", fps=values["speed_slider"]
        )
...
while True:
    ...
    if event == sg.WINDOW_CLOSED:
        ...
        parameter_sliders["aleph0_low"].update(
            disabled=not values["override_aleph_checkbox"]
        )
        parameter_sliders["aleph0_high"].update(
            disabled=not values["override_aleph_checkbox"]
        )
    elif (
        running
        and (time.monotonic() - wait) > 1 / values["speed_slider"]
        and not terminated
    ):
        step()
        wait = time.monotonic()
