...
class SimpleGridworld(MDPWorldModel):
    ...
    *Deltas* (rewards) can accrue from the following events:
    ...
    def __init__(
        ...
        self.delta_xygrid = delta_xygrid = (
            np.array(delta_grid).T
            if delta_grid is not None
            else np.full(xygrid.shape, " ")
        )
        ...
        self._window_shape = (
            800 * np.array(xygrid.shape) / np.max(xygrid.shape)
        )  # The size of the PyGame window in pixels
        ...
        self.n_immobile_objects = (
            self.n_mobile_constant_objects
        ) = self.n_mobile_variable_objects = 0  # TODO: extract from grid
        ...
        for x in range(xygrid.shape[0]):
            for y in range(xygrid.shape[1]):
                if xygrid[x, y] in immobile_object_types:
                    ...
                    self.immobile_object_state0_deltas.append(
                        cell_code2delta[delta_xygrid[x, y]]
                        if delta_xygrid[x, y] != " "
                        else 0
                    )
                    ...
                    self.mobile_constant_object_deltas.append(
                        cell_code2delta[delta_xygrid[x, y]]
                        if delta_xygrid[x, y] != " "
                        else 0
                    )
                    ...
                    self.mobile_variable_object_state0_deltas.append(
                        cell_code2delta[delta_xygrid[x, y]]
                        if delta_xygrid[x, y] != " "
                        else 0
                    )
    ...
    def _can_move(self, from_loc, to_loc, state, who="A"):
        ...
        if not (
            0 <= to_loc[0] < self.xygrid.shape[0]
            and 0 <= to_loc[1] < self.xygrid.shape[1]
            and self.xygrid[to_loc] not in unenterable_immobile_cell_types
        ):
            return False
        ...
        (
            ...
            mv_locs,
            mv_states,
        ) = self._extract_state_attributes(state)
            ...
            if to_loc == (mc_locs[2 * i], mc_locs[2 * i + 1]):
                ...
                if object_type in ["X", "|"]:  # a box
                    if who != "A" and (
                        object_type == "X"
                        or (
                            object_type == "|" and from_loc[1] != to_loc[1]
                        )  # attempt to push glass pane up or down
                    ):
    ...
    def state_embedding(self, state):
        res = np.array(state_embedding_for_distance(state), dtype=np.float32)[
            3:
        ]  # make time and previous position irrelevant
        return res
    ...
    def possible_actions(self, state=None):
        ...
        if len(actions) == 0:
            raise ValueError(
                f"No possible actions from state {state}"
            )  # FIXME: raise a more specific exception
        return actions
    ...
    def _extract_state_attributes(self, state, gridcontents=False):
        ...
        t, loc, prev_loc, imm_states, mc_locs, mv_locs, mv_states = (
            ...
            (state[3], state[4]),  # current location
            (state[1], state[2]),  # previous location
            state[5 : 5 + self.n_immobile_objects],  # immobile object states
            state[
                5 + self.n_immobile_objects : 5
                + self.n_immobile_objects
                + 2 * self.n_mobile_constant_objects
            ],  # mobile constant object locations
            state[
                5 + self.n_immobile_objects + 2 * self.n_mobile_constant_objects : 5
                + self.n_immobile_objects
                + 2 * self.n_mobile_constant_objects
                + 2 * self.n_mobile_variable_objects
            ],  # mobile variable object locations
            state[
                5
                + self.n_immobile_objects
                + 2 * self.n_mobile_constant_objects
                + 2 * self.n_mobile_variable_objects : 5
                + self.n_immobile_objects
                + 2 * self.n_mobile_constant_objects
                + 3 * self.n_mobile_variable_objects
            ],  # mobile variable object states
        )
        if not gridcontents:
            return t, loc, prev_loc, imm_states, mc_locs, mv_locs, mv_states
        gc = {
            get_loc(mc_locs, i): (self.mobile_constant_object_types[i], i)
    ...
    def state_distance(self, state1, state2):
        ...
        return np.sqrt(
            np.sum(
                np.power(
                    np.array(state_embedding_for_distance(state1))[1:]
                    - np.array(state_embedding_for_distance(state2))[1:],
                    2,
                )
            )
        )
    ...
    def transition_distribution(self, state, action, n_samples=None):
        ...
        if at_goal:
            successor = self._make_state(
                t + 1, loc, loc, imm_states, mc_locs, mv_locs, mv_states
            )
            return {successor: (1, True)}
        else:
            if cell_type == ",":
                ...
                if imm_states[self.immobile_object_indices[loc]] == 0:
                    # turn state to 1:
                    imm_states = set_entry(
                        imm_states, self.immobile_object_indices[loc], 1
                    )
            ...
            if target_type in ["^", "~"]:
                ...
                simulated_actions = [
                    ...
                    != self.opposite_action(
                        action
                    )  # won't fall back to where we came from
                    and self._can_move(
                        target_loc, self._get_target_location(target_loc, a), state
                    )
                ]
                if len(simulated_actions) > 0:
                    p0 = (
                        1 if target_type == "^" else self.uneven_ground_prob
                    )  # probability of falling off
                    intermediate_state = self._make_state(
                        t, target_loc, loc, imm_states, mc_locs, mv_locs, mv_states
                    )
                    trans_dist = {}
                    ...
                    for simulate_action in simulated_actions:
                        for successor, (probability, _) in self.transition_distribution(
                            intermediate_state, simulate_action, n_samples
                        ).items():
                ...
                trans_dist = {
                    self._make_state(
                        t + 1, target_loc, loc, imm_states, mc_locs, mv_locs, mv_states
                    ): 1
                }  # stay in the same state with probability 1
                ...
                for i, object_type in enumerate(self.mobile_constant_object_types):
                    ...
                    if object_type == "F":  # a fragile object
                        if (
                            object_loc != (-2, -2) and self.move_probability_F > 0
                        ):  # object may move
                            ...
                            for successor, probability in trans_dist.items():
                                (
                                    ...
                                    succ_mv_states,
                                    gridcontents,
                                ) = self._extract_state_attributes(
                                    successor, gridcontents=True
                                )
                                if object_loc == target_loc:  # object is destroyed
                                    default_successor = self._make_state(
                                        succ_t,
                                        succ_loc,
                                        succ_prev_loc,
                                        succ_imm_states,
                                        set_loc(succ_mc_locs, i, (-2, -2)),
                                        succ_mv_locs,
                                        succ_mv_states,
                                    )
                                ...
                                direction_locs = [
                                    (
                                        direction,
                                        self._get_target_location(object_loc, direction),
                                    )
                                    for direction in range(4)
                                ]
                                direction_locs = [
                                    (direction, loc)
                                    for (direction, loc) in direction_locs
                                ...
                                if n_directions == 0:
                                    ...
                                    new_trans_dist[default_successor] = probability * (
                                        1 - self.move_probability_F
                                    )
                                    p = (
                                        probability
                                        * self.move_probability_F
                                        / n_directions
                                    )
                                    for direction, obj_target_loc in direction_locs:
                                        if (
                                            obj_target_loc == target_loc
                                        ):  # object is destroyed
                                            new_successor = self._make_state(
                                                succ_t,
                                                succ_loc,
                                                succ_prev_loc,
                                                succ_imm_states,
                    ...
                    for y in range(self.xygrid.shape[1]):
                        ...
                        if len(values) > 0:  # then it is a list
                            ...
                            canvas.blit(
                                surf,
                                (
                                    (x + 0.5) * pix_square_size - 0.5 * surf.get_width(),
