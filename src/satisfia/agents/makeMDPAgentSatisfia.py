...
class AspirationAgent(ABC):
    ...
    def __init__(self, params):
        ...
        defaults = {
            ...
            "fifthMomentOfDelta": (
                lambda state, action: 8
                * self.params["varianceOfDelta"](state, action) ** 2.5
            ),  # assumes a Gaussian distribution
            "sixthMomentOfDelta": (
                lambda state, action: 15
                * self.params["varianceOfDelta"](state, action) ** 3
            ),  # assumes a Gaussian distribution
            "debug": None,
            "verbose": None,
        }
        ...
        assert (
            self.params["allowNegativeCoeffs"] or self.params["lossCoeff4Random"] >= 0
        ), "lossCoeff4random must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"]
            or self.params["lossCoeff4FeasibilityPower"] >= 0
        ), "lossCoeff4FeasibilityPower must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"] or self.params["lossCoeff4DP"] >= 0
        ), "lossCoeff4DP must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"]
            or self.params["lossCoeff4AgencyChange"] >= 0
        ), "lossCoeff4AgencyChange must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"] or self.params["lossCoeff4LRA1"] >= 0
        ), "lossCoeff4LRA1 must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"] or self.params["lossCoeff4Time1"] >= 0
        ), "lossCoeff4Time1 must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"] or self.params["lossCoeff4Entropy1"] >= 0
        ), "lossCoeff4Entropy1 must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"] or self.params["lossCoeff4KLdiv1"] >= 0
        ), "lossCoeff4KLdiv1 must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"] or self.params["lossCoeff4Variance"] >= 0
        ), "lossCoeff4variance must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"] or self.params["lossCoeff4Fourth"] >= 0
        ), "lossCoeff4Fourth must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"] or self.params["lossCoeff4Cup"] >= 0
        ), "lossCoeff4Cup must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"] or self.params["lossCoeff4LRA"] >= 0
        ), "lossCoeff4LRA must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"] or self.params["lossCoeff4Time"] >= 0
        ), "lossCoeff4time must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"]
            or self.params["lossCoeff4DeltaVariation"] >= 0
        ), "lossCoeff4DeltaVariation must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"]
            or self.params["lossCoeff4WassersteinTerminalState"] >= 0
        ), "lossCoeff4WassersteinTerminalState must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"] or self.params["lossCoeff4Entropy"] >= 0
        ), "lossCoeff4entropy must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"] or self.params["lossCoeff4KLdiv"] >= 0
        ), "lossCoeff4KLdiv must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"]
            or self.params["lossCoeff4TrajectoryEntropy"] >= 0
        ), "lossCoeff4TrajectoryEntropy must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"]
            or self.params["lossCoeff4StateDistance"] >= 0
        ), "lossCoeff4StateDistance must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"] or self.params["lossCoeff4Causation"] >= 0
        ), "lossCoeff4Causation must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"]
            or self.params["lossCoeff4CausationPotential"] >= 0
        ), "lossCoeff4CausationPotential must be >= 0"
        assert (
            self.params["allowNegativeCoeffs"] or self.params["lossCoeff4OtherLoss"] >= 0
        ), "lossCoeff4OtherLoss must be >= 0"

        if "defaultPolicy" not in self.params:
            self.params["defaultPolicy"] = self.world.default_policy
        assert (
            self.params["lossCoeff4Entropy"] == 0
            or self.params["lossCoeff4DP"] == 0
            or ("uninformedPolicy" in self.params)
        ), "uninformedPolicy must be provided if lossCoeff4DP > 0 or lossCoeff4Entropy > 0"
        assert self.params["lossCoeff4StateDistance"] == 0 or (
            "referenceState" in self.params
        ), "referenceState must be provided if lossCoeff4StateDistance > 0"

        self.debug = DEBUG if self.params["debug"] is None else self.params["debug"]
        self.verbose = (
            VERBOSE if self.params["verbose"] is None else self.params["verbose"]
        )
    ...
    def maxAdmissibleV(self, state):  # recursive
        ...
        if actions != []:
            ...
            v = (
                max(qs)
                if self["maxLambda"] == 1
                else interpolate(min(qs), self["maxLambda"], max(qs))
            )
    ...
    def minAdmissibleV(self, state):  # recursive
        ...
        if actions != []:
            ...
            v = (
                min(qs)
                if self["minLambda"] == 0
                else interpolate(min(qs), self["minLambda"], max(qs))
            )
    ...
    def aspiration4state(self, state, unclippedAleph):
        ...
        res = clip(
            self.minAdmissibleV(state),
            Interval(unclippedAleph),
            self.maxAdmissibleV(state),
        )
        if self.verbose or self.debug:
            print(
                pad(state),
    ...
    def disorderingPotential_state(self, state):  # recursive
        ...
        actions = self.possible_actions(state)
        maxMPpolicyWeights = [
            math.exp(self.disorderingPotential_action(state, a)) for a in actions
        ]
        if sum(maxMPpolicyWeights) == 0:
            return 0  # TODO this shouldn't be 0
        res = math.log(sum(maxMPpolicyWeights))
        if self.debug or self.verbose:
            print(
                pad(state),
                "| | ╰ disorderingPotential_state",
                prettyState(state),
                ":",
                res,
            )
    ...
    def _compute_default_transition(self, state):
        ...
        def default_transition(source):
            ...
            if (
                len(targets) == 0
            ):  # use a uniform distribution over all possible successors
                targets = self.world.possible_successors(source)
                return distribution.categorical(targets, [1 for target in targets])
            else:
                return distribution.categorical(
                    targets, [math.exp(the_scores[target]) for target in targets]
                )
    ...
    def agency_state(self, state):  # recursive
        ...
        if self.world.is_terminal(state):
            ...
            def X(other_state):
                aps = [
                    (a, self.world.transition_probability(state, a, other_state)[0])
                    for a in actions
                ]
    ...
    def localPolicyData(self, state, aleph):
        ...
        assert actions != []
        alephs = [
            self.aspiration4action(state, action, aleph4state) for action in actions
        ]
        ...
        def propensity(indices, estAlephs):
            p = 1 / len(indices)
            losses = [
                self.combinedLoss(state, actions[index], aleph4state, estAlephs[index], p)
                for index in indices
            ]  # bottleneck
            min_loss = min(losses)
            return [
                max(math.exp(-(loss - min_loss) / self["lossTemperature"]), 1e-100)
                for loss in losses
            ]
        ...
        for i1, p1 in distribution.categorical(indices, propensities).categories():
            ...
            if Interval(adm1) <= Interval(aleph4state):
                ...
                indices2 = [
                    index
                    for index in indices
                    if between(midTarget, midpoint(alephs[index]), mid1)
                ]
                if len(indices2) == 0:
                    print(
                        "OOPS: indices2 is empty",
                        a1,
                        adm1,
                        aleph4state,
                        midTarget,
                ...
                for i2, p2 in distribution.categorical(
                    indices2, propensities2
                ).categories():
        ...
        ps = [
            max(1e-100, locPol.probability(item)) for item in support
        ]  # 1e-100 prevents normalization problems
    ...
    def propagateAspiration(self, state, action, aleph4action, Edel, nextState):
        ...
        lam = relativePosition(
            self.minAdmissibleQ(state, action),
            aleph4action,
            self.maxAdmissibleQ(state, action),
        )  # TODO didn't we calculate the admissible Q when we chose the action?
        ...
        rescaledAleph4nextState = interpolate(
            self.minAdmissibleV(nextState), lam, self.maxAdmissibleV(nextState)
        )
    ...
    def relativeQ2(self, s, a, al, v):  # aleph4action
        ...
        res = self.Q2(s, a, al) - 2 * self.Q(s, a, al) * v + v**2
        if self.debug or self.verbose:
    ...
    def cupLoss_state(self, state, unclippedAleph):  # recursive
        ...
        def X(actionAndAleph):
            return self.cupLoss_action(
                state, actionAndAleph[0], aleph4state, actionAndAleph[1]
            )

        res = self.localPolicy(state, aleph4state).expectation(X)
        if self.debug or self.verbose:
            print(
                pad(state), "╰ cupLoss_state", prettyState(state), aleph4state, ":", res
            )
        return res
    ...
    def LRAdev_state(self, state, aleph4state):  # recursive
        ...
        def X(actionAndAleph):
            return self.LRAdev_action(
                state, actionAndAleph[0], actionAndAleph[1]
            )  # recursion
    ...
    def ETerminalState_state(self, state, aleph4state, policy="actual"):  # recursive
        ...
        if self.world.is_terminal(state):
            ...
            def X(actionAndAleph):
                return self.ETerminalState_action(
                    state, actionAndAleph[0], actionAndAleph[1], policy
                )  # recursion
    ...
    def ETerminalState2_state(self, state, aleph4state, policy="actual"):  # recursive
        ...
        if self.world.is_terminal(state):
            ...
            def X(actionAndAleph):
                return self.ETerminalState2_action(
                    state, actionAndAleph[0], actionAndAleph[1], policy
                )  # recursion
    ...
    def behaviorEntropy_state(self, state, aleph4state):  # recursive
        ...
        def X(actionAndAleph):
            return self.behaviorEntropy_action(
                state,
                locPol.probability(actionAndAleph),
                actionAndAleph[0],
                actionAndAleph[1],
            )  # recursion
    ...
    def behaviorKLdiv_state(self, state, aleph4state):  # recursive
        ...
        def X(actionAndAleph):
            return self.behaviorKLdiv_action(
                state,
                locPol.probability(actionAndAleph),
                actionAndAleph[0],
                actionAndAleph[1],
    ...
    def trajectoryEntropy_state(self, state, aleph4state):  # recursive
        ...
        def X(actionAndAleph):
            return self.trajectoryEntropy_action(
                state,
                locPol.probability(actionAndAleph),
                actionAndAleph[0],
                actionAndAleph[1],
    ...
    def stateDistance_state(self, state, aleph4state):  # recursive
        ...
        def X(actionAndAleph):
            return self.stateDistance_action(
                state, actionAndAleph[0], actionAndAleph[1]
            )  # recursion
    ...
    def causation_state(self, state, aleph4state):  # recursive
        ...
        def Y(nextState, action):
            ...
            return math.log(
                p
                / locPol.expectation(
                    lambda otherActionAndAleph: self.world.transition_probability(
                        state, otherActionAndAleph[0], nextState
                    )[0]
                )
            )
        ...
        def X(actionAndAleph):
            ...
            return self.world.expectation(
                state, action, Y, (action,)
            ) + self.causation_action(
                state, aleph4state, action, aleph4action
            )  # recursion
    ...
    def causationPotential_state(self, state, aleph4state):  # recursive
        ...
        raise NotImplementedError(
            "causationPotential_state is not yet implemented correctly"
        )
        if self.debug:
            print(
                pad(state),
                "causationPotential_state",
                prettyState(state),
                aleph4state,
                "...",
            )
        ...
        res = max(
            [
                self.world.expectation(state, action, Y, (action,))
                + self.causationPotential_action(
                    state,
                    aleph4state,
                    action,
                    self.aspiration4action(state, action, aleph4state),
                )
    ...
    def combinedLoss(self, s, a, al4s, al4a, p):  # recursive
        ...
        lRandom = expr_params(
            lambda l: l * self.randomTieBreaker(s, a), "lossCoeff4Random"
        )
        lFeasibilityPower = expr_params(
            lambda l: l * (self.maxAdmissibleQ(s, a) - self.minAdmissibleQ(s, a)) ** 2,
            "lossCoeff4FeasibilityPower",
        )
        lDP = expr_params(
            lambda l: l * self.disorderingPotential_action(s, a), "lossCoeff4DP"
        )
        lAgencyChange = expr_params(
            lambda l: l * self.agencyChange_action(s, a), "lossCoeff4AgencyChange"
        )
        lLRA1 = expr_params(
            lambda l: l * self.LRAdev_action(s, a, al4a, True), "lossCoeff4LRA1"
        )
        lEntropy1 = expr_params(
            lambda l: l * self.behaviorEntropy_action(s, p, a), "lossCoeff4Entropy1"
        )
        lKLdiv1 = expr_params(
            lambda l: l * self.behaviorKLdiv_action(s, p, a), "lossCoeff4KLdiv1"
        )
        ...
        estVs = midpoint(al4s)
        lVariance = expr_params(
            lambda l: l * self.relativeQ2(s, a, al4a, estVs), "lossCoeff4Variance"
        )  # recursion
        lFourth = expr_params(
            lambda l: l * self.relativeQ4(s, a, al4a, estVs), "lossCoeff4Fourth"
        )  # recursion
        lCup = expr_params(
            lambda l: l * self.cupLoss_action(s, a, al4s, al4a), "lossCoeff4Cup"
        )  # recursion
        lLRA = expr_params(
            lambda l: l * self.LRAdev_action(s, a, al4a), "lossCoeff4LRA"
        )  # recursion

        # timing-related criteria:
        q_ones = expr_params(
            lambda x, y: self.Q_ones(s, a, al4a),
            "lossCoeff4DeltaVariation",
            "lossCoeff4Time",
        )
        lTime = expr_params(lambda l: l * q_ones, "lossCoeff4Time")
        lDeltaVariation = 0
        if q_ones != 0:
            lDeltaVariation = expr_params(
                lambda l: l
                * (
                    self.Q_DeltaSquare(s, a, al4a) / q_ones
                    - self.Q2(s, a, al4a) / (q_ones**2)
                ),
                "lossCoeff4DeltaVariation",
            )  # recursion

        # change-related criteria:
        lWassersteinTerminalState = expr_params(
            lambda l: l * self.wassersteinTerminalState_action(s, a, al4a),
            "lossCoeff4WassersteinTerminalState",
        )  #

        # randomization-related criteria:
        lEntropy = expr_params(
            lambda l: l * self.behaviorEntropy_action(s, p, a, al4a),
            "lossCoeff4Entropy",
        )  # recursion
        lKLdiv = expr_params(
            lambda l: l * self.behaviorKLdiv_action(s, p, a, al4a), "lossCoeff4KLdiv"
        )  # recursion
        lTrajectoryEntropy = expr_params(
            lambda l: l * self.trajectoryEntropy_action(s, p, a, al4a),
            "lossCoeff4TrajectoryEntropy",
        )  # recursion
        lStateDistance = expr_params(
            lambda l: l * self.stateDistance_action(s, a, al4a),
            "lossCoeff4StateDistance",
        )  # recursion
        lCausation = expr_params(
            lambda l: l * self.causation_action(s, al4s, a, al4a), "lossCoeff4Causation"
        )  # recursion
        lCausationPotential = expr_params(
            lambda l: l * self.causationPotential_action(s, al4s, a, al4a),
            "lossCoeff4CausationPotential",
        )  # recursion

        lOther = 0
        if "otherLocalLoss" in self.params:
            lOther = expr_params(
                lambda l: l * self.otherLoss_action(s, a, al4a), "lossCoeff4OtherLoss"
            )  # recursion
    ...
    @abstractmethod
    def Q(self, state, action, aleph4action):
        pass
    ...
    @abstractmethod
    def possible_actions(self, state, action):
        pass
...
class AgentMDPLearning(AspirationAgent):
    def __init__(
        ...
        Q_ones=None,
        Q_DeltaSquare=None,
        behaviorEntropy_action=None,
        behaviorKLdiv_action=None,
        trajectoryEntropy_action=None,
        stateDistance_action=None,
        ...
        Q5=None,
        Q6=None,
        ETerminalState_action=None,
        ETerminalState2_action=None,
...
class AgentMDPPlanning(AspirationAgent):
    ...
    def disorderingPotential_action(self, state, action):  # recursive
        ...
        def f(nextState, probability):
            if self.world.is_terminal(nextState):
                ...
                internalEntropy = (
                    self["internalTransitionEntropy"](state, action, nextState)
                    if self["internalTransitionEntropy"]
                    else 0
                )
    ...
    def agencyChange_action(self, state, action):  # recursive
        ...
        def f(successor):
            return (
                0
                if self.world.is_terminal(successor)
                else abs(math.log(state_agency) - math.log(self.agency_state(successor)))
            )
    ...
    def Q(self, state, action, aleph4action):  # recursive
        if self.debug:
            print(
                pad(state), "| | | | Q", prettyState(state), action, aleph4action, "..."
            )
        ...
        def total(nextState):
            if self.world.is_terminal(nextState):
                ...
                nextAleph4state = self.propagateAspiration(
                    state, action, aleph4action, Edel, nextState
                )
        ...
        def total(nextState):
            if self.world.is_terminal(nextState):
                ...
                return (
                    ...
                    + self.V2(nextState, nextAleph4state)
                )  # recursion
        ...
        def total(nextState):
            if self.world.is_terminal(nextState):
                ...
                return (
                    ...
                    + self.V3(nextState, nextAleph4state)
                )  # recursion
        ...
        def total(nextState):
            if self.world.is_terminal(nextState):
                ...
                return (
                    ...
                    + self.V4(nextState, nextAleph4state)
                )  # recursion
        ...
        def total(nextState):
            if self.world.is_terminal(nextState):
                ...
                return (
                    ...
                    + self.V5(nextState, nextAleph4state)
                )  # recursion
        ...
        def total(nextState):
            if self.world.is_terminal(nextState):
                ...
                return (
                    ...
                    + self.V6(nextState, nextAleph4state)
                )  # recursion
    ...
    def LRAdev_action(self, state, action, aleph4action, myopic=False):  # recursive
        ...
        def dev(nextState):
            ...
            if self.world.is_terminal(nextState) or myopic:
                ...
                return localLRAdev + self.LRAdev_state(
                    nextState, nextAleph4state
                )  # recursion
    ...
    def Q_DeltaSquare(self, state, action, aleph4action=None):  # recursive
        ...
        def d(nextState):
            if self.world.is_terminal(nextState) or aleph4action is None:
                ...
                return EdelSq + self.V_DeltaSquare(
                    nextState, nextAleph4state
                )  # recursion
    ...
    @lru_cache(maxsize=None)
    def ETerminalState_action(
        self, state, action, aleph4action, policy="actual"
    ):  # recursive
        if self.debug:
            print(
                pad(state),
                "| | | | | ETerminalState_action",
                prettyState(state),
                action,
                aleph4action,
                policy,
        ...
        if policy == "actual":
            ...
            def X(nextState):
                ...
                return self.ETerminalState_state(
                    nextState, nextAleph4state, policy
                )  # recursion
    ...
    @lru_cache(maxsize=None)
    def ETerminalState2_action(
        self, state, action, aleph4action, policy="actual"
    ):  # recursive
        if self.debug:
            print(
                pad(state),
                "| | | | | ETerminalState2_action",
                prettyState(state),
                action,
                aleph4action,
                policy,
        ...
        if policy == "actual":
            ...
            def X(nextState):
                ...
                return self.ETerminalState2_state(
                    nextState, nextAleph4state, policy
                )  # recursion
    ...
    def wassersteinTerminalState_action(self, state, action, aleph4action):
        if self.debug:
            print(
                pad(state),
                "| | | | wassersteinTerminalState_action",
                prettyState(state),
                action,
                aleph4action,
                "...",
    ...
    def behaviorEntropy_action(
        self, state, actionProbability, action, aleph4action=None
    ):  # recursive
        ...
        def entropy(nextState):
            uninfPolScore = (
                self["uninformedPolicy"](state).score(action)
                if ("uninformedPolicy" in self.params)
                else 0
            )
            localEntropy = (
                uninfPolScore
                ...
                + (
                    self["internalActionEntropy"](state, action)
                    if ("internalActionEntropy" in self.params)
                    else 0
                )
            )
            if self.world.is_terminal(nextState) or aleph4action is None:
                ...
                return localEntropy + self.behaviorEntropy_state(
                    nextState, nextAleph4state
                )  # recursion
    ...
    def behaviorKLdiv_action(
        ...
        def div(nextState):
            ...
            if self.world.is_terminal(nextState) or aleph4action is None:
                ...
                return localDivergence + self.behaviorKLdiv_state(
                    nextState, nextAleph4state
                )  # recursion
    ...
    def trajectoryEntropy_action(
        ...
        def entropy(nextState, transitionProbability):
            ...
            localEntropy = (
                ...
                + (
                    self["internalTrajectoryEntropy"](state, action)
                    if ("internalTrajectoryEntropy" in self.params)
                    else 0
                )
            )
            ...
            if self.world.is_terminal(nextState) or aleph4action is None:
                ...
                return localEntropy + self.trajectoryEntropy_state(
                    nextState, nextAleph4state
                )  # recursion
    ...
    def stateDistance_action(self, state, action, aleph4action=None):  # recursive
        ...
        def X(nextState):
            if self.world.is_terminal(nextState) or aleph4action is None:
                return (
                    self.world.state_distance(nextState, self.params["referenceState"])
                    ** 2
                )
            else:
                nextAleph4state = self.propagateAspiration(
    ...
    # @lru_cache(maxsize=None)
    def causation_action(
        self, state, aleph4state, action, aleph4action=None
    ):  # recursive
    ...
    def causationPotential_action(
        ...
        def X(nextState):
            if self.world.is_terminal(nextState) or aleph4action is None:
                ...
                return self.causationPotential_state(
                    nextState, nextAleph4state
                )  # recursion
    ...
    def otherLoss_action(self, state, action, aleph4action=None):  # recursive
        ...
        def loss(nextState):
            ...
            if self.world.is_terminal(nextState) or aleph4action is None:
                ...
                return localLoss + self.otherLoss_state(
                    nextState, nextAleph4state
                )  # recursion
