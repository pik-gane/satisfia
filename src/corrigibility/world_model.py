...
class NodeData:
    ...
    node_belief: NodeDistribution = field(default_factory=dict)
    ...
    policy_belief: Policy = field(default_factory=dict)
    ...
    action_labels: ActionLabels = field(default_factory=dict)
    ...
    durations: Dict[ActionId, Duration] = field(default_factory=dict)
    ...
    """(optional) A list of action probabilities (for agent and probabilistic players), keyed by action_id, or None if this is a terminal node. For probabilistic players, this must be present from the outset, while for the agent player, this is initially None and will be set by the agent during its planning."""
    ...
    """(optional) The principal's belief about its power in this terminal node, as assumed by the agent because of the non-modelled future after this terminal node of the model, or None if this is not a terminal node"""

    actual_power: Power = None 
    """(optional) The actual power of the principal in this terminal node, as assumed by the agent because of the modelled future after this terminal node of the model, or None if this is not a terminal node"""


class WorldModel:
    """A type of extensive-form game form with imperfect information and additional data (see above)"""
...
#    @cached
    def get_assumed_principal_policy_and_more(self, 
            ...
            last_principal_node_id: NodeId = None,  # the last decision node of the principal, defaulting to actual_node_id
        ...
        Note: currently restricted to world models with perfect information."""
        ...
        last_principal_node = self.node_data(last_principal_node_id or actual_node_id)
        ...
        if player in self.principal_players:
            ...
            for action in actions:
                ...
                duration = actual_node.durations.get(action, 1)
                ...
                for goal in actual_node.goal_prior:
                    ...
                    a = action_attainment_probabilities[(action, goal)] = (1.0 if successor_id in goal_nodes else later_attprobs[goal].get(successor_id, 0.0)) * np.exp(-goal_rate * duration)
            ...
            for action in actions:
                ...
                successor_id = actual_node.consequences[action]
                later_policy, _, _, later_powers, later_attprobs = self.get_assumed_principal_policy_and_more(successor_id, last_principal_node_id, override_policy_belief)
                # update policy, powers, and attprobs with the values of the successor node:
                principal_policy.update(later_policy)
                powers.update(later_powers)
                duration = actual_node.durations.get(action, 1)
                discounted_power = (powers.get(successor_id, None) or 1) * np.exp(-actual_node.power_discount_rate * duration)
                ...
                for goal in last_principal_node.goal_prior:
                    ...
                    successor_attprob = 1.0 if successor_id in goal_nodes else later_attprobs.get(goal, {}).get(successor_id, 0.0)
            ...
            for action in actions:
                ...
                for goal in last_principal_node.goal_prior:
                    goal_nodes, goal_rate = goal
                    attainment_probabilities[goal] = attainment_probabilities.get(goal, {})
                    successor_attprob = 1.0 if successor_id in goal_nodes else later_attprobs.get(goal, {}).get(successor_id, 0.0) * np.exp(-goal_rate * duration)
...
@dataclass
class SimpleExample(WorldModel):

    players = ["Cora", "Prince"]
    agent_player = "Cora"
    principal_players = ["Prince"]
    
    _node_data = {
        "root": NodeData(player="Prince", 
                                    action_labels={"pass": "pass", "swerve!": "say 'swerve'"}, 
                                    consequences={"pass": "passed", "swerve!": "spoke"},
                                    goal_prior={
                                        (frozenset(["passed,passed", "spoke,passed"]),0): 3/4,
                                        (frozenset(["passed,swerved", "spoke,swerved"]),0): 1/4,
                                    }),
        "passed": NodeData(player="Cora",
                                      action_labels={"pass": "pass", "swerve": "swerve"},
                                      consequences={"pass": "passed,passed", "swerve": "passed,swerved"}),
        "spoke": NodeData(player="Cora",
                                      action_labels={"pass": "pass", "swerve": "swerve"},
                                      consequences={"pass": "spoke,passed", "swerve": "spoke,swerved"})
    }
...
if __name__ == "__main__":
    ND = NodeData
    Pas = {"foo": "foo", "bar": "bar"}
    Aas = {"act": "act", "pass": "pass"}
    g = WorldModel(players=["A","P","O","R"], principal_players=["P"], probabilistic_players=["R"], initial_node="0", nodes={
        "0": ND(player="P", action_labels=Pas, consequences={"foo": "0-0", "bar": "0-1"}, goal_prior={
            (frozenset(["0-0-a"]),0): 1/4,
            (frozenset(["0-0-p"]),0): 1/4,
            (frozenset(["0-1-a"]),0): 1/4,
            (frozenset(["0-1-p"]),0): 1/4,
            }),
        "0-0": ND(player="A", action_labels=Aas, consequences={"act": "0-0-a", "pass": "0-0-p"}),
        "0-1": ND(player="A", action_labels=Aas, consequences={"act": "0-1-a", "pass": "0-1-p"}),
        "0-0-a": ND(),
        "0-0-p": ND(),
        "0-1-a": ND(),
        "0-1-p": ND()
    ...
    print(g._node_data)
    print(g.get_assumed_principal_policy_and_more("0"))