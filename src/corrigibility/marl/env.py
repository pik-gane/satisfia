...
try:
    from .envs.map_loader import DEFAULT_MAP, load_map
    from .objects import (
        CHAR_TO_OBJ_CLASS,
        COLORS,
        TILE_PIXELS,
...
class CustomEnvironment(ParallelEnv):
    ...
    def __init__(
        ...
        grid_metadata=None,
        render_mode=None,
    ):  # Added grid_layout and grid_metadata parameters
        super().__init__()
        self.debug_mode = debug_mode  # Store debug_mode
        self.debug_level = debug_level  # Store debug_level
        self.render_mode = render_mode

        # Load map layout and metadata - either from file or directly provided
        if grid_layout is not None and grid_metadata is not None:
            # Use directly provided grid layout and metadata
            self.map_layout = grid_layout
            self.map_metadata = grid_metadata
        else:
            # Load from map file
        ...
        # Calculate actual observation space size after environment is fully initialized
        # This will be updated in _initialize_observation_space after parsing the map
        self._observation_spaces = {}

        # Initialize placeholders - will be updated in _initialize_observation_space
        self._obs_size = None
        self._action_spaces = {
            agent: Discrete(len(Actions)) for agent in self.possible_agents
        }
        ...
        # Initialize observation space now that we know the map structure
        self._initialize_observation_space()

    def _initialize_observation_space(self):
        """Initialize observation space based on the actual map structure."""
        # Parse the map to determine the number of doors, keys, and boxes
        self._parse_map_layout()

        # Calculate the actual observation size to match the observe method:
        # 1. Agent direction (1 element)
        # 2. All agent positions (2 * num_agents elements)
        # 3. Door states (num_doors elements)
        # 4. Key states (2 * num_keys elements)
        # 5. Robot inventory (2 * num_keys elements for picked up keys)
        # 6. Box positions (2 * num_boxes elements)

        obs_size = 1  # Agent direction
        obs_size += 2 * len(self.possible_agents)  # All agent positions
        obs_size += len(self.doors)  # Door states
        obs_size += 2 * len(self.keys)  # Key positions
        obs_size += 2 * len(self.boxes)  # Box positions
        ...
        high_values = []
        # Agent direction (0-3)
        high_values.append(4)
        # Agent positions (0 to grid_size-1 for each coordinate)
        for _ in range(len(self.possible_agents)):
            high_values.extend([self.grid_size, self.grid_size])
        # Door states (0 or 1)
        high_values.extend([2] * len(self.doors))
        # Key states (positions: 0 to grid_size-1, or special values: -2 for inventory, -1 for padding)
        ...
        self._observation_spaces = {
            agent: MultiDiscrete(high_values) for agent in self.possible_agents
        }

    def _is_cardinally_adjacent(
    ...
    def _parse_map_layout(self):
        ...
        for r, row in enumerate(self.map_layout):
            for c, code in enumerate(row):
                ...
                if (
                    grid_r < 0
                    or grid_r >= self.grid_size
                    or grid_c < 0
                    or grid_c >= self.grid_size
                ):
                    continue
    ...
    def _get_obs(self, agent_id):  # agent_id is for context, obs is global
        ...
        obs_agent_pos = (
            self.agent_pos if self.agent_pos is not None else (-1, -1)
        )  # Robot position
        obs_human_pos = (
            self.human_pos if self.human_pos is not None else (-1, -1)
        )  # Human position
        obs_agent_dir = self.agent_dirs.get(
            self.robot_agent_ids[0], 0
        )  # Robot direction
        obs_human_dir = self.agent_dirs.get(
            self.human_agent_ids[0], 0
        )  # Human direction
    ...
    def _is_valid_pos(self, pos):
        ...
        # block locked doors
        for door in self.doors:
            if door["pos"] == pos and not door["is_open"]:
                return False
    ...
    def _handle_movement(self, agent_to_move_id, new_pos):
        ...
        current_pos = self.agent_positions[
            agent_to_move_id
        ]  # Use the definitive position
        ...
        is_goal_tile = any(
            current_pos == goal_pos for goal_pos in self.human_goals.values()
        )
        if is_goal_tile:
            old_char_on_grid = "G"
        ...
        if (
            agent_to_move_id in self.robot_agent_ids
            and agent_to_move_id == self.robot_agent_ids[0]
        ):
            self.agent_pos = new_pos
        if (
            agent_to_move_id in self.human_agent_ids
            and agent_to_move_id == self.human_agent_ids[0]
        ):
    ...
    def step(self, actions: dict[str, int]):
        ...
        prev_potentials_h = {hid: self.potential(hid) for hid in self.human_agent_ids}
        ...
        for agent in order:  # 'agent' is the agent_id string e.g. "robot_0", "human_0"
            ...
            if (
                agent in self.human_agent_ids
            ):  # Simplified: all humans have same restriction
            ...
            current_pos = self.agent_positions[
                agent
            ]  # CORRECTED: Use agent_positions for current agent
            ...
            other_agent_positions = [
                pos for aid, pos in self.agent_positions.items() if aid != agent
            ]
            ...
            # Forward moves the agent in its facing direction if valid
                ...
                collision_with_other_agent = any(
                    front_pos == other_pos for other_pos in other_agent_positions
                )
                if self._is_valid_pos(front_pos) and not collision_with_other_agent:
                    self._handle_movement(
                        agent, front_pos
                    )  # Pass the specific agent_id
                ...
                if action_enum == Actions.pickup:
                    for key in list(self.keys):
                        if key["pos"] == front_pos:
                            self.robot_has_keys.add(
                                key["color"]
                            )  # Assuming one robot's inventory for now
                    ...
                    for door in self.doors:
                        if (
                            door["pos"] == front_pos
                            and door["is_locked"]
                            and door["color"] in self.robot_has_keys
                        ):
                            door["is_locked"] = False
                            door["is_open"] = True
                            self.grid[front_pos] = (
                                " "  # Door becomes passable, visually handled by Door object state
                            )
                            break
                elif action_enum == Actions.drop and self.robot_has_keys:
                    occupied_by_objects_or_agents = (
                        {tuple(d["pos"]) for d in self.doors}
                        | {tuple(k["pos"]) for k in self.keys}
                        | set(self.agent_positions.values())
                    )
                    if (
                        0 <= front_pos[0] < self.grid_size
                        and 0 <= front_pos[1] < self.grid_size
                        and self.grid[front_pos] == " "  # Can only drop on empty floor
                        and front_pos not in occupied_by_objects_or_agents
        ...
        for hid in self.human_agent_ids:
            ...
            prev_potential = prev_potentials_h.get(
                hid, current_potential_h
            )  # Default to current if no prev (e.g. first step)
            ...
            if (
                self.debug_mode
                and self.debug_level == "verbose"
                and abs(shaped_reward) > 0.01
            ):
                print(
                    f"🔄 SHAPED REWARD: Human {hid} got {shaped_reward:.3f} (potential: {prev_potential:.2f} → {current_potential_h:.2f})"
                )
        ...
        for hid in self.human_agent_ids:
            if self.agent_positions.get(hid) == self.human_goals.get(hid):
                ...
                if (
                    not hasattr(self, "_humans_completed")
                    or hid not in self._humans_completed
                ):
                    ...
                    if self.debug_mode or hasattr(self, "_minimal_debug_enabled"):
                        ...
                        print(
                            f"🎯 GOAL REACHED: Human {hid} reached goal {self.human_goals.get(hid)} at step {self.timestep}{episode_info}"
                        )
        ...
        if len(humans_at_goals) == len(self.human_agent_ids):
            ...
            if self.debug_mode or hasattr(self, "_minimal_debug_enabled"):
                ...
                print(
                    f"🏆 ALL GOALS COMPLETED: All humans reached their goals at step {self.timestep}{episode_info}"
                )
        ...
        if self.timestep >= self.max_steps:
            ...
            if self.debug_mode:  # Conditional debug print
                print(
                    f"DEBUG: Max steps {self.max_steps} reached. Truncating."
                )  # DEBUG
    ...
    def print_debug_info(self):
        ...
        for agent_id in self.possible_agents:
            ...
            print(
                f"DEBUG: Agent {agent_id} | Pos: {pos} | Reward: {reward:.2f} | Term: {termination} | Trunc: {truncation}"
            )  # Formatted reward
            if agent_id.startswith("human"):
                goal = self.human_goals.get(agent_id, "N/A")

                # Enhanced logging for PrevPotential
                prev_potential_val_for_log = prev_potentials_h.get(
                    agent_id
                )  # Try direct get first
                if prev_potential_val_for_log is not None and isinstance(
                    prev_potential_val_for_log, (float, int)
                ):
                ...
                print(
                    f"DEBUG: Human {agent_id} | Goal: {goal} | PrevPotential: {prev_potential_str} | CurrPotential: {current_potential_val_for_log:.2f}"
                )
    ...
    def render(self):
        ...
        if self.screen is None and self.render_mode == "human":
            ...
            self.agent_char_font = pygame.font.SysFont(
                "Arial", int(self.cell_size * 0.5)
            )  # reduced size from 0.75
        ...
        for r in range(self.grid_size):
            for c in range(self.grid_size):
                ...
                if agent_char_to_render:
                    temp_tile_surface = pygame.surfarray.make_surface(
                        np.transpose(tile_img_np, (1, 0, 2))
                    )

                    text_surface = self.agent_char_font.render(
                        agent_char_to_render, True, char_color
                    )
                ...
                tile_surface = pygame.surfarray.make_surface(
                    np.transpose(tile_img_np, (1, 0, 2))
                )
        ...
        text_panel_rect = pygame.Rect(
            0, self.grid_viz_size, self.window_width, self.text_panel_height
        )
    ...
    def update_robot_reward_for_logging(
        self, robot_agent_id: str, internal_reward: float
    ):
    ...
    def get_all_possible_human_goals(self):
        return self.get_human_goals()
