...
class QLearningBackend(ABC):
    ...
    def __init__(self, agent_ids: List[str], action_space_dict: Dict[str, List[int]], 
                 state_dim: int, debug: bool = False):
    ...
    @abstractmethod
    def update_q_values(self, agent_id: str, state: Tuple, action: int, target: float, 
                       learning_rate: float, goal: Optional[Tuple] = None):
        """Update Q-values for a given state-action pair."""
        pass
    
    @abstractmethod
    def get_policy(self, agent_id: str, state: Tuple, temperature: float, 
                   goal: Optional[Tuple] = None) -> Dict[int, float]:
...
class TabularQLearning(QLearningBackend):
    ...
    def __init__(self, agent_ids: List[str], action_space_dict: Dict[str, List[int]], 
                 state_dim: int, debug: bool = False, use_goals: bool = True,
                 beta_h: float = 5.0, policy_update_rate: float = 0.1, nu_h: float = 0.1):
        ...
        if use_goals:
            # For humans: Q(s, g, a)
            self.q_tables = {
                aid: defaultdict(lambda: np.random.uniform(-0.1, 0.1, size=len(Actions)))
                for aid in agent_ids
            }
        else:
            # For robots: Q(s, a)
            self.q_tables = {
                aid: defaultdict(lambda: np.random.uniform(-0.1, 0.1, size=len(action_space_dict[aid])))
                for aid in agent_ids
            }
    ...
    def update_q_values(self, agent_id: str, state: Tuple, action: int, target: float, 
        ...
        self.update_policy_smooth(agent_id, state, q_values, self.beta_h, self.policy_update_rate, self.nu_h, goal)
    ...
    def update_policy_smooth(self, agent_id: str, state: Tuple, q_values: np.ndarray, 
                           temperature: float, update_rate: float, nu_h: float = 0.1,
        ...
        pi_softmax_dict = {allowed_actions[i]: pi_softmax[i] for i in range(len(allowed_actions))}
        ...
        for a in allowed_actions:
            ...
            pi_new[a] = (1 - update_rate) * pi_old.get(a, pi_norm[a]) + update_rate * mixed_policy
    ...
    def update_policy_direct(self, agent_id: str, state: Tuple, q_values: np.ndarray, 
                           temperature: float, goal: Optional[Tuple] = None):
        ...
        for agent_id in self.agent_ids:
            for key, q_values in self.q_tables[agent_id].items():
                ...
                if self.use_goals and isinstance(key, tuple) and len(key) == 2:
                    state, goal = key
                    self.update_policy_direct(agent_id, state, q_values, temperature, goal)
    ...
    def load_models(self, filepath: str):
        ...
        for aid, qtable_dict in data['q_tables'].items():
            if self.use_goals:
                ...
                q_table = defaultdict(lambda: np.zeros(len(self.action_space_dict[aid])))
...
class NetworkQLearning(QLearningBackend):
    ...
    def __init__(self, agent_ids: List[str], action_space_dict: Dict[str, List[int]], 
        ...
        for aid in agent_ids:
            ...
            # Main network
            self.networks[aid] = QNetwork(self.input_dim, output_dim, hidden_sizes).to(self.device)
            
            # Target network for stability
            self.target_networks[aid] = QNetwork(self.input_dim, output_dim, hidden_sizes).to(self.device)
            self.target_networks[aid].load_state_dict(self.networks[aid].state_dict())
            
            # Optimizer
            self.optimizers[aid] = torch.optim.Adam(self.networks[aid].parameters(), lr=0.001)
    ...
    def update_q_values(self, agent_id: str, state: Tuple, action: int, target: float, 
        ...
        if self.update_counter % self.target_update_freq == 0:
            self.target_networks[agent_id].load_state_dict(self.networks[agent_id].state_dict())
    ...
    def save_models(self, filepath: str):
        ...
        checkpoint = {
            ...
            'target_networks': {aid: net.state_dict() for aid, net in self.target_networks.items()},
    ...
    def load_models(self, filepath: str):
        ...
        for aid in self.agent_ids:
            ...
            self.target_networks[aid].load_state_dict(checkpoint['target_networks'][aid])
...
def create_q_learning_backend(use_networks: bool, agent_ids: List[str], 
                            action_space_dict: Dict[str, List[int]], state_dim: int,
                            use_goals: bool = True, debug: bool = False, **kwargs) -> QLearningBackend:
    """Factory function to create appropriate Q-learning backend."""
    if use_networks:
        return NetworkQLearning(agent_ids, action_space_dict, state_dim, debug, use_goals, **kwargs)
    else:
        return TabularQLearning(agent_ids, action_space_dict, state_dim, debug, use_goals, **kwargs)
