...
class Door(WorldObj):
    def __init__(
        self, color: str = "brown", is_open: bool = False, is_locked: bool = False
    ):
    ...
    def toggle(self, env, pos):  # env is the MiniGridEnv instance
        if self.is_locked:
            ...
            if (
                hasattr(env, "robot_has_key") and env.robot_has_key
            ):  # Check if env has this attribute
    ...
    def render(self, img):
        ...
        if self.is_open:
            ...
            door_tile_color = (
                COLORS["grey"] * 0.3
            )  # Very light grey, almost transparent
            ...
            fill_coords(
                img,
                point_in_circle(keyhole_center_x, keyhole_center_y, keyhole_radius),
                COLORS["black"],
            )
            fill_coords(
                img,
                point_in_rect(
                    keyhole_center_x - 0.05,
                    keyhole_center_x + 0.05,
                    keyhole_center_y,
                    keyhole_center_y + 0.25,
                ),
                COLORS["black"],
            )
        else:
            ...
            fill_coords(
                img, point_in_rect(0.0, 1.0, 0.0, 1.0), self.color
            )  # Fill entire tile with base color
            draw_handle = True  # Draw handle for closed, unlocked door

        if (
            not is_transparent_middle and not self.is_locked
        ):  # Avoid drawing over keyhole or open door center
            fill_coords(img, point_in_rect(0.0, 1.0, 0.0, 1.0), door_tile_color)

        if (
            draw_handle
        ):  # Draw handle if applicable (closed & unlocked, or originally intended for open)
            ...
            fill_coords(
                ...
                point_in_rect(
                    handle_x_start,
                    handle_x_start + handle_width,
                    handle_y_start,
                    handle_y_start + handle_height,
                ),
                COLORS["black"],
            )
        ...
        if not self.is_open:
            fill_coords(
                img, point_in_rect(0, 1, 0, 0.05), COLORS["black"]
            )  # Top border
            fill_coords(
                img, point_in_rect(0, 1, 0.95, 1), COLORS["black"]
            )  # Bottom border
            fill_coords(
                img, point_in_rect(0, 0.05, 0.05, 0.95), COLORS["black"]
            )  # Left border
            fill_coords(
                img, point_in_rect(0.95, 1, 0.05, 0.95), COLORS["black"]
            )  # Right border
